...
Waifu Finder API client, generated from the OpenAPI spec.

This API allows users to generate and explore anime-style images with simple queries.

Learn more about Waifu Finder API at [github.com/l0v3m0n3y/waifufinder](https://github.com/l0v3m0n3y/waifufinder).

This is an unofficial SDK for the Waifu Finder public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
```bash
cd go-mcp && go build -o waifu-finder-mcp .
```
...
```json
{
  "mcpServers": {
    "waifu-finder": {
      "command": "/abs/path/to/waifu-finder-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **Image** | The Image entity (list). | `/images/random` |
...
$images = $client->Image()->list();
print_r(array_map(fn($item) => $item->data_get(), $images));
```

### Golang

```go
import sdk "github.com/voxgig-sdk/waifu-finder-sdk/go"
...
- Upstream API: [https://github.com/l0v3m0n3y/waifufinder](https://github.com/l0v3m0n3y/waifufinder)
...
Generated from the Waifu Finder API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
