# WaifuFinder Ruby SDK



The Ruby SDK for the WaifuFinder API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
require_relative "WaifuFinder_sdk"
client = WaifuFinderSDK.new(options)
```
...
```ruby
client = WaifuFinderSDK.test(testopts, sdkopts)
```
...
raise a `WaifuFinderError` (a `StandardError` subclass), so wrap
...
The main module (`WaifuFinder_sdk`) exports the SDK class
