...
require_relative 'WaifuFinder_types'
...
class WaifuFinderSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = WaifuFinderHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WaifuFinderHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WaifuFinderFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WaifuFinderUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WaifuFinderHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WaifuFinderHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WaifuFinderHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = WaifuFinderHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WaifuFinderError.new(
        "graphql_error", "WaifuFinderSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = WaifuFinderSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
