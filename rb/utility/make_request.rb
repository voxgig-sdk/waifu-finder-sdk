...
module WaifuFinderUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WaifuFinderResponse.new({})
    ...
    if fetch_err
      ...
      response = WaifuFinderResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WaifuFinderResponse.new(fetched)
    else
