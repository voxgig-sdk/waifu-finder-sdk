...
class ImageEntity
  ...
  def data_set(args)
    if args
      @_data = WaifuFinderHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = WaifuFinderHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Image>, Array] the matching Image items; raises WaifuFinderError on failure
