...
class ImageEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = WaifuFinderSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set WAIFUFINDER_TEST_IMAGE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def image_basic_setup(extra)
  ...
  entid_env_raw = ENV["WAIFUFINDER_TEST_IMAGE_ENTID"]
  ...
  if env["WAIFUFINDER_TEST_LIVE"] == "TRUE"
    ...
    client = WaifuFinderSDK.new(Helpers.to_map(merged_opts))
  end
