...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.WAIFUFINDER_TEST_LIVE ||
    'TRUE' === process.env.WAIFUFINDER_TEST_OVERRIDE
  ) {
  ...
  m.WAIFUFINDER_TEST_EXPLAIN = process.env.WAIFUFINDER_TEST_EXPLAIN || m.WAIFUFINDER_TEST_EXPLAIN
