...
function isRunnable(code) {
    return (/new\s+WaifuFinderSDK\b/.test(code) ||
        /\bWaifuFinderSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+WaifuFinderSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/WaifuFinderSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
