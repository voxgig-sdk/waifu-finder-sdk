...
class WaifuFinderTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WaifuFinderTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WaifuFinderTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WaifuFinderTestRunner.getenv("WAIFU_FINDER_TEST_LIVE")
        override = WaifuFinderTestRunner.getenv("WAIFU_FINDER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WaifuFinderTestRunner.getenv(key)
        ...
        explain = WaifuFinderTestRunner.getenv("WAIFU_FINDER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if WaifuFinderTestRunner._test_control is not None:
            return WaifuFinderTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WaifuFinderTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WaifuFinderTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WaifuFinderTestRunner.load_test_control()
...
def load_env_local():
    WaifuFinderTestRunner.load_env_local()


def env_override(m):
    return WaifuFinderTestRunner.env_override(m)


def entity_data(v):
    return WaifuFinderTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WaifuFinderTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WaifuFinderTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WaifuFinderTestRunner.load_test_control()


def live_client_options():
    return WaifuFinderTestRunner.live_client_options()


def live_delay_ms():
    return WaifuFinderTestRunner.live_delay_ms()
