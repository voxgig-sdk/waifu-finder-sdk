...
from waifufinder_sdk.utility.voxgig_struct import voxgig_struct as vs
...
from waifufinder_sdk.core.response import WaifuFinderResponse
from waifufinder_sdk.core.operation import WaifuFinderOperation
from waifufinder_sdk.core.error import WaifuFinderError
from waifufinder_sdk.core import helpers
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return WaifuFinderError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_exists(self):
        client = WaifuFinderSDK.test(None, None)
    ...
    def test_feature_init_basic(self):
        init_client = WaifuFinderSDK.test(None, None)
        ...
        class TestInitFeature(WaifuFinderBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(WaifuFinderBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = WaifuFinderSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = WaifuFinderSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = WaifuFinderOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = WaifuFinderSDK.test(None, setup_opts)
