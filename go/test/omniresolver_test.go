...
import (
	...
	omni "github.com/voxgig-sdk/waifu-finder-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WaifuFinderError); is {
				out := map[string]any{
					"name":    "WaifuFinderError",
...
func omniCtx(arg any, client *sdk.WaifuFinderSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WaifuFinderSDK); is {
			client = live
