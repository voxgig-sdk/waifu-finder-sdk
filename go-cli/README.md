...
boru-driven command-line client **and** interactive REPL for the WaifuFinder
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/waifu-finder-cli)
make build

# 2. See usage (words, entities, env vars)
./waifu-finder-cli --help

# 3. Provide credentials once, via the environment
export WAIFU_FINDER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./waifu-finder-cli list image

# 5. Override the API base URL for a single call
WAIFU_FINDER_BASE=https://api.example.com ./waifu-finder-cli list image

# 6. No arguments -> interactive REPL
./waifu-finder-cli
waifu-finder> list image
waifu-finder> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/waifu-finder-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/waifu-finder-cli list image
   ```
...
Run with no arguments to open a REPL (prompt `waifu-finder>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/waifu-finder-cli`. |
...
(`./waifu-finder-cli <expr>`) and interactively (the REPL), and expressions compose the
