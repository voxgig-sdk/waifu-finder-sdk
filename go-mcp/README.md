...
[MCP](https://modelcontextprotocol.io) server exposing the WaifuFinder SDK as
two agent tools — `waifu-finder_list` and `waifu-finder_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/waifu-finder-mcp)
make build

# 2. Provide credentials via the environment
export WAIFU_FINDER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user waifu-finder \
  -- /absolute/path/to/waifu-finder-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./waifu-finder-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// waifu-finder_list: first page of records
{ "entity": "image" }
{ "entity": "image", "query": { } }

// waifu-finder_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/waifu-finder-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user waifu-finder \
     -- "$PWD"/dist/*/waifu-finder-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `waifu-finder_list` and `waifu-finder_load` tools now appear
   in new sessions. Ask the agent to *"list image using waifu-finder"*
   and it calls `waifu-finder_list` with `{"entity":"image"}`.
...
```sh
export WAIFU_FINDER_APIKEY=sk_live_xxx            # API key
export WAIFU_FINDER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `waifu-finder_list` tool
...
### Call the `waifu-finder_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `waifu-finder_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `waifu-finder_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WAIFU_FINDER_APIKEY` | API key sent with every request. |
| `WAIFU_FINDER_BASE` | Optional override of the API base URL. |
...
```sh
./waifu-finder-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"waifu-finder_load","arguments":{"entity":"image","query":{"id":1}}}}'
```
