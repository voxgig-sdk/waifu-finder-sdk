...
class FtCtrl extends WaifuFinderControl
{
...
class FtClient
{
    ...
    private WaifuFinderUtility $utility;

    public function __construct(WaifuFinderUtility $utility, array $options)
    {
    ...
    public function get_utility(): WaifuFinderUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WaifuFinderContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WaifuFinderContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new WaifuFinderUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (WaifuFinderContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WaifuFinderConfig::make_feature($name);
    ...
    private static function buildUrl(WaifuFinderSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WaifuFinderError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WaifuFinderResponse($fetched);
        }
    ...
    private function fail(WaifuFinderContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WaifuFinderContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
