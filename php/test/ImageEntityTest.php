...
class ImageEntityTest extends TestCase
{
    ...
    {
        $testsdk = WaifuFinderSDK::test(null, null);
    ...
    public function test_stream(): void
    {
        ...
        $base = WaifuFinderSDK::test($seed, null);
        $seen = iterator_to_array($base->Image(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = WaifuFinderConfig::make_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = WaifuFinderSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set WAIFUFINDER_TEST_IMAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function image_basic_setup($extra)
{
    ...
    $client = WaifuFinderSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("WAIFUFINDER_TEST_IMAGE_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "WAIFUFINDER_TEST_IMAGE_ENTID" => $idmap,
        "WAIFUFINDER_TEST_LIVE" => "FALSE",
        "WAIFUFINDER_TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["WAIFUFINDER_TEST_IMAGE_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["WAIFUFINDER_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new WaifuFinderSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["WAIFUFINDER_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
