...
// WaifuFinder SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class WaifuFinderMakeError
{
    public static function call(?WaifuFinderContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new WaifuFinderContext([], null);
        }
        $op = $ctx->op ?? new WaifuFinderOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new WaifuFinderResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof WaifuFinderError) ? $err->msg : (string)$err;
        $msg = "WaifuFinderSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new WaifuFinderError('', $msg, $ctx);
        ...
        if ($err instanceof WaifuFinderError) {
