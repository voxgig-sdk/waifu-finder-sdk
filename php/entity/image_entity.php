...
class ImageEntity
{
    ...
    {
        if ($args) {
            $this->_data = WaifuFinderHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = WaifuFinderHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws WaifuFinderError on failure (item-5 convention).
     */
