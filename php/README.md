...
The PHP SDK for the WaifuFinder API — an entity-oriented client using PHP conventions.
...
```php
<?php
require_once 'waifufinder_sdk.php';

$client = new WaifuFinderSDK();
```
...
print_r(array_map(fn($item) => $item->data_get(), $image));
```
...
```php
require_once 'waifufinder_sdk.php';
$client = new WaifuFinderSDK($options);
```
...
The main class (`waifufinder_sdk.php`) exports the SDK class
