...
module WaifuFinderUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = WaifuFinderContext.new({}, nil)
    end
    op = ctx.op || WaifuFinderOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || WaifuFinderResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(WaifuFinderError) ? err.msg : err.to_s
    ...
    sdk_err = WaifuFinderError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(WaifuFinderError)
